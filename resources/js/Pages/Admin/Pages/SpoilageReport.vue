<template>
    <AdminLayout>
        <AppHeader title=" Spoilage Maintenance Report" />
...
<script setup>
const props = defineProps({ spoilages: Object, branches: Object });
...
props.spoilages.forEach((element) => {
    reportData.push({
...
const selectBranchHandler = (id) => {
    router.get(`/spoilage_report_branch/${id}`);
};
...
const exportToCSV = () => {
    if (!props.spoilages) return;
    try {
        const fileName = "exported-data";
        const exportType = "csv";
        const data = props.spoilages;
