<template>
    <AdminLayout>
        <AppHeader title="Checklist Report" />
            ...
            <div id="element-to-convert">
                ...
                <table class="ml-appMiddeumSpace mr-appMiddeumSpace">
                    ...
                    <tr
                        ...
                        class="text-[12px]"
                    >
                        <td>{{ body.MainChecklist }}</td>
                        <td>{{ body.SubChecklist }}</td>
                        <td>{{ body.Task }}</td>
                        <td>{{ body.User }}</td>
                        <td>{{ body.Branch }}</td>
...
<script setup>
const props = defineProps({ checklists: Object, branches: Object });
...
props.checklists.forEach((item) => {
    reportData.push({
        id: item.id,
        MainChecklist: item.main_check_list.name,
        SubChecklist: item.sub_checklist.name,
        Task: item.checklist_task.task,
        User: item.user.name,
...
const selectBranchHandler = (id) => {
    router.get(`/checklist_report_branch/${id}`);
};
const currentDate = () => {
